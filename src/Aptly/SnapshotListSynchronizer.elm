effect module Aptly.SnapshotListSynchronizer where { subscription = MySub, command = MyCmd } exposing (modify, onModify)
...
type MyCmd msg
    = Modify (Aptly.Generic.List.Modification Aptly.Snapshot.Snapshot)

type MySub msg
    = OnModify (Aptly.Generic.List.Modification Aptly.Snapshot.Snapshot -> msg)
...
type SelfMsg
    = Modification (Aptly.Generic.List.Modification Aptly.Snapshot.Snapshot)

cmdMap : (a -> b) -> MyCmd a -> MyCmd b
cmdMap tagger (Modify modification) =
    Modify modification

modify : Aptly.Generic.List.Modification Aptly.Snapshot.Snapshot -> Cmd msg
modify data =
    command <| Modify data

onModify : (Aptly.Generic.List.Modification Aptly.Snapshot.Snapshot -> msg) -> Sub msg
onModify tagger =
